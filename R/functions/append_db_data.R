  ...
  result <- tryCatch(
    ...
    error = function(e) {
      if (
        str_detect(
          e$message,
          'duplicate key value violates unique constraint "parcels_pid_unique"'
        )
      ) {
      ...
      # Return FALSE instead of the error object
      return(FALSE)
    }
  )

  # If result is FALSE, return early without showing success message
  if (isFALSE(result)) {
    return(FALSE)
  } else {
