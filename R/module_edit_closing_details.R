      ...
      layout_sidebar(
        ...
        sidebar = sidebar(
          ...
          actionButton(
            ...
            label = "Update Conservation Lands",
            class = "btn-primary"
          ),
          downloadButton(
            outputId = ns("download_cons_lands"),
            label = "Downlad Conservation Lands Shapefile",
            class = "btn-primary"
          )
        ),
        ## Main panel ----
...
module_edit_closing_details_server <- function(id, db_con, db_updated = NULL) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$create_cons_lands, {
      ...
      cons_lands_attribs <- dbGetQuery(
        db_con,
        "SELECT * FROM view_conservation_lands_spatial;"
      ) |>
        as_tibble()
    ...
    output$download_cons_lands <- downloadHandler(
      ...
      content = function(fname) {
        map_shp_names <- c(
          ...
          size_notes = "size_confirmed_notes",
